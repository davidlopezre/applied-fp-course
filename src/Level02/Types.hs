...
-- - plain text
-- - json
data ContentType = PlainTextContent | JsonTypeContent
...
renderContentType JsonTypeContent = "application/json"
...
mkTopic
  :: Text
  -> Either Error Topic
mkTopic t = if t == "" then Left EmptyText else Right (Topic t)
...
mkCommentText t = if t == "" then Left EmptyText else Right (CommentText t)
